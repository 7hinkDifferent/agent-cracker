...
- [x] **multi-coder** — 多 Coder 多态架构（工厂模式 + 12 种编辑格式运行时切换）
- [x] **chat-summary** — LLM 驱动的历史摘要（二分递归压缩 + 后台线程异步执行）
- [x] **fuzzy-match** — 多级容错匹配（精确 → 空白容忍 → 省略号 → 编辑距离 80%）
- [x] **context-window** — 上下文窗口管理（token 采样估算 + 二分搜索约束 + 三层降级）

## 完整串联

- [x] **mini-aider** — 组合以上 MVP 组件的最小完整 agent

## 进度

MVP: 4/4 | 进阶: 7/7 | 串联: 1/1 | 总计: 12/12
