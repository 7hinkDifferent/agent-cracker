...
### vs aider / codex-cli / pi-agent / nanoclaw / eigent

| 维度 | openclaw | aider | codex-cli | pi-agent | nanoclaw | eigent |
|------|----------|-------|-----------|----------|----------|--------|
| **定位** | 多通道 AI 助手平台 | 终端编码助手 | CLI 编码 agent | 模块化 agent 工具包 | 极简个人 AI 助手 | 桌面多 Agent Workforce 平台 |
| **语言** | TypeScript | Python | Rust + TS | TypeScript | TypeScript | TypeScript + Python |
| **Agent Loop** | 内嵌 pi-agent + 编排层 | 三层嵌套 | tokio 多路复用 | 双层循环 + steering | 双层：Host 轮询 + Container SDK | 队列事件循环 + Workforce 并行编排 |
| **Tool 数量** | 47+ 核心 + plugin | 7 文本命令 | 6 function calling | 7 原生 tool calling | SDK 内置 + 6 MCP tool | 30+ Toolkit + MCP + Skill |
| **Tool 管理** | 4 档 profile + policy | 无分级 | 3 级审批 | 无分级 | SDK 全权限 + IPC 权限分级 | @listen_toolkit 事件织入 + 按 Agent 类型过滤 |
| **通道** | 13+ 消息平台 | 仅 CLI | 仅 CLI | CLI + Slack Bot | WhatsApp + skill 扩展 | Electron 桌面 + Webhook + Slack 触发 |
| **记忆** | Vector + BM25 混合检索 | Git 集成 | 无 | 无（session 内 compaction） | CLAUDE.md 文件 + SQLite session | PostgreSQL 聊天历史 + Redis + Qdrant |
| **安全** | Docker 沙箱 + 信任分级 | Git auto-commit | 平台沙箱 + 网络代理 | 无 | Docker 容器隔离 + 外部 allowlist | JWT + 速率限制（无代码沙箱） |
| **调度** | Cron + Heartbeat | 无 | 无 | 无 | Cron/Interval/Once | Celery Beat + Webhook 触发 |
| **子 Agent** | spawn + steer + kill | 无 | 无 | 无 | Agent Swarms（Claude SDK Teams） | 8 类 Agent Workforce 并行 |
| **扩展** | Plugin SDK + 31 extension + 51 skills | 无 | Hooks + MCP | 深度扩展系统 | Claude Code Skills（代码变换） | Skill 多层配置 + MCP 管理 |
| **代码规模** | ~450,000 行 | ~30,000 行 | ~30,000 行 | ~25,000 行 | ~3,900 行 | ~50,000+ 行（含前端） |
| **框架依赖** | 内嵌 pi-agent | 自研 | 自研 | 自研 | Claude Agent SDK | CAMEL-AI（重度依赖） |
