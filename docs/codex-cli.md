...
| 维度 | codex-cli | aider | pi-agent |
|------|-----------|-------|----------|
| **语言** | Rust + TypeScript | Python | TypeScript |
| **Agent Loop** | tokio::select! 多路复用 + turn 循环 | 三层嵌套（外层切换 + REPL + 反思） | 双层循环（steering + follow-up） |
| **Tool 系统** | 原生 function calling + 协议驱动 + 审批门 | 双轨制：用户命令 + LLM 文本格式 | 原生 tool calling + pluggable ops |
| **Context 策略** | bytes/4 估算 + 首尾保留截断 + auto-compact | tree-sitter AST + PageRank RepoMap | chars/4 估算 + 结构化摘要压缩 |
| **编辑方式** | apply_patch（unified diff） | 12+ 编辑格式多态切换 | edit tool（精确替换 + 模糊匹配） |
| **安全模型** | 三级审批 + 平台沙箱 + 网络代理 | Git 集成（自动 commit + undo） | 无内建沙箱 |
| **错误处理** | 可重试性分类 + 指数退避 | 反思循环 + 多级解析容错 | 多 provider overflow 检测 |
| **扩展性** | Hooks + MCP + Skills + Custom Prompts | 无正式扩展系统 | 深度扩展（生命周期钩子） |
| **多模态** | 支持图片输入 | 不支持 | 不支持 |
| **Session** | 无明显持久化 | Git 集成（auto-commit） | JSONL 持久化 + 分支 |
| **LLM 支持** | OpenAI 为主（可配置） | litellm 统一适配 | 原生多 provider SDK |
...
与 Aider 相比，Codex CLI 更重**安全与执行控制**（沙箱、审批、网络策略），Aider 更重**代码理解智能**（RepoMap、多编辑格式、反思循环）。与 pi-agent 相比，Codex CLI 有硬件级沙箱和网络代理，pi-agent 有更灵活的环境抽象（Pluggable Ops）和实时交互（Steering Queue）。Codex CLI 适合需要高安全性和自主执行的场景（CI/CD 集成、全自动修复），Aider 适合深度代码编辑，pi-agent 适合高度自定义的嵌入场景。
